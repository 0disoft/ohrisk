...
describe("main", () => {
  ...
  test("prints package version", async () => {
    ...
    expect(exitCode).toBe(0);
    expect(stderr).toEqual([]);
    expect(stdout).toEqual(["ohrisk 0.160.13"]);
  });
  ...
  test("opens Windows HTML reports with the default URL handler", async () => {
  ...
  test("prints SARIF report with stable rule ids and lockfile locations", async () => {
    ...
    expect(payload.runs[0]?.tool.driver.semanticVersion).toBe("0.160.13");
