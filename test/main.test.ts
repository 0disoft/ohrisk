...
describe("main", () => {
  ...
  test("prints package version", async () => {
    ...
    expect(exitCode).toBe(0);
    expect(stderr).toEqual([]);
    expect(stdout).toEqual(["ohrisk 0.160.2"]);
  });
  ...
  test("prints actionable findings for a Python uv.lock project with local source evidence", async () => {
  ...
  test("keeps Windows HTML report opening alive after explorer returns a non-zero status", async () => {
    const outputRoot = mkdtempSync(path.join(tmpdir(), "ohrisk-html-windows-open-"));
    const outputPath = path.join(outputRoot, "scan.html");
    writeFileSync(outputPath, "<!doctype html><title>windows fixture report</title>", "utf8");
    let openCommand = "";
    let openedTarget = "";
    let fetchedText = "";
    let fetchedTextPromise: Promise<void> | undefined;
    ...
    try {
      ...
      expect(openCommand).toBe("explorer.exe");
      expect(opened.ok).toBe(true);
      expect(opened.ok ? opened.value.target : "").toBe(openedTarget);
      expect(fetchedText).toContain("windows fixture report");
    } finally {
      rmSync(outputRoot, { recursive: true, force: true });
    }
  });
  ...
  test("prints SARIF report with stable rule ids and lockfile locations", async () => {
    ...
    expect(payload.runs[0]?.tool.driver.semanticVersion).toBe("0.160.2");
...
`,
  ...
  test("diff reads baseline uv.lock local source metadata", async () => {
    const projectRoot = mkdtempSync(path.join(tmpdir(), "ohrisk-uv-local-source-diff-"));
    const localSourceDir = path.join(projectRoot, "local-risk");
    const lockfile = [
      "version = 1",
      "revision = 3",
    ...
    try {
      mkdirSync(localSourceDir, { recursive: true });
      writeFileSync(path.join(projectRoot, "uv.lock"), lockfile, "utf8");
      ...
      expect(exitCode).toBe(0);
      expect(stderr).toEqual([]);
      expect(requestedBaselinePaths).toContain("uv.lock");
