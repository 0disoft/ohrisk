...
- Standalone HTML reports now use a compact review dashboard with persistent
  ...
  identities while severity and explanatory fields appear once in the detail
  inspector. Finding selection is keyboard accessible, filtered results keep
  the inspector synchronized, and machine report values and waiver identities
  remain unchanged.
...
- The active SPDX `Unicode-3.0` identifier is now classified as permissive
  ...
  obligation.
- The active SPDX `PSF-2.0` identifier is now classified as permissive low-risk
  evidence while retaining the distributed license-text preservation
  obligation.
