...
- **SPDX catalog validation**: standard license and exception terms are checked against the pinned official catalog. Unlisted identifiers fail closed instead of being treated as high-confidence SPDX metadata; deprecated identifiers remain compatible.
...
- **Active findings**: filterable severity, search, dependency, and action controls with detail cards for Severity, Package, Dependency, Reason, Action, Path, Evidence, and Fingerprint. Long detail values are collapsed by default and can be expanded in the browser. Search text is derived once from visible card text when the report opens instead of being duplicated in per-card attributes. Fingerprints of 512 characters or more are stored once in a script-inert JSON dictionary and shared path-prefix trie, then reconstructed exactly when their collapsed detail is expanded. Short fingerprints remain inline. This changes only standalone HTML representation: finding IDs, waiver matching, and JSON, Markdown, SARIF, and CycloneDX values are unchanged.
