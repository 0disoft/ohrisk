...
- **Bundled-component reconciliation**: when a primary `LICENSE` or `LICENCE` file has a qualified sibling such as `LICENSE.libyaml`, `LICENSE.thirdparty`, `LICENSE.vendor`, or `LICENSE.component`, the qualified file is cumulative component evidence. Its recognized expression is combined with the package expression using SPDX `AND`; document extensions and explicit license variants remain ordinary package-license evidence.
