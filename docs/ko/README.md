...
루트에 지원 입력이 없으면 Ohrisk가 중첩 dependency project 하나를 자동 선택하거나,
여러 project의 지원 입력을 lockfile provenance와 함께 저장소 전체 graph로 합친다.
자동 병합은 최대 64개 project root와 128개 입력으로 제한된다. 특정 입력만 검사하려면
안전한 저장소 상대 `--lockfile` 경로로 범위를 좁힐 수 있다. 같은 project root의
`pom.xml`과 `requirements.txt`도 자동으로 함께 처리한다. 절대 경로와 상위 경로 이탈은
거부한다.
Maven 좌표는 로컬 POM evidence를 쓸 수 없을 때 먼저 고정된 공식
`repo.maven.apache.org` host에서 정확한 버전의 POM과 최대 8단계 parent POM을
조회한다. project `pom.xml`에 선언된 다른 HTTPS repository는 그 정확한 hostname이
host policy 또는 `--allow-host`로 허용된 경우에만 조회한다. POM identity, XML, 응답
크기, timeout, redirect, DNS/socket 주소를 검증하고 npm registry token은 전달하지
않는다. POM chain에 license가 없으면 같은 repository의 SHA-256 checksum과 JAR 내부
Maven identity가 모두 일치할 때만 root/META-INF license file을 evidence로 사용한다.
...
Git 서브모듈은 가져오지 않고, 심볼릭 링크는 대상을 따라가지 않은 채 checkout에서
제거한다. 둘 다 제한된 상대 경로와 함께 불완전한 검사 범위로 보고되며, 심볼릭
링크는 lockfile이나 manifest 입력으로 사용할 수 없다.
Windows 예약 이름이나 대소문자 충돌처럼 지원 환경에서 표현할 수 없는 일반 파일은
checkout 전에 제외하고 별도 경로로 보고한다. 경로 탈출과 `.git` 경로는 계속 실패한다.
...
[CLI command contract](../cli/command-contract.md#archive-input)를 따른다.
...
npm package-lock/shrinkwrap graph 순회는 반복형으로 수행하며 도달 가능한 패키지를
모두 유지한다. 패키지 하나에 저장하는 dependency path는 최대 64개이고, 초과하면
typed truncation diagnostic을 남긴다.
...
package source record와 전체 Git commit으로 해석된 원격 Git record를 지원한다. 원격 Git
source는 clone하거나 같은 이름의 PyPI 패키지로 바꾸지 않고 unavailable evidence로 남겨
정확한 commit의 license를 직접 확인하게 한다. `uv.lock` 의존성 순회는 도달 가능한 package를
모두 유지하되 package마다 경로를 최대 64개만 저장하고, 초과 경로는
`dependency_paths_truncated` 진단으로 알린다. `Pipfile.lock`은 `default`와 `develop` 섹션의 정확한
...
license 파일을 evidence로 읽는다. 고정된 PyPI package는 정확한 버전의 PyPI release
metadata에서 SHA-256이 있는 sdist 또는 wheel을 선택하고, digest와 내부 name/version을
검증한 뒤 `PKG-INFO`, `METADATA`, license 파일을 원격 evidence로 읽는다.
`requirements.txt`는 `name==version`처럼
버전이 고정된 package, 프로젝트 root 안의 local source entry, `-e ./local-package`
같은 editable local source entry, `-r base.txt` 같은 include, `-c constraints.txt`의
정확한 constraint pin을 지원한다. pip-compile이 만든 `# via` 주석에 이름이 나온
parent package가 같은 파일에 있으면 direct/transitive 경로도 제한된 깊이와 개수 안에서
복원한다. parent를 확인할 수 없는 주석은 안전하게 직접 의존성으로 취급한다.
로컬 source package는 `pyproject.toml`,
`setup.cfg`, `PKG-INFO`의 name/version/license metadata와 root license 파일을
evidence로 읽는다. 전체 commit으로 해석되지 않은 uv 원격 VCS entry, Pipenv/PDM 원격 VCS entry, 프로젝트 root 밖 uv/Pipenv/PDM local
source path, 원격 VCS `requirements.txt` entry, 정확한 constraint pin이 없는 unpinned range는 아직
...
스캔하고, 로컬 `.m2/repository`의 POM license metadata를 먼저 읽은 뒤 없으면
정확한 버전의 Maven Central POM evidence를 사용한다.
Maven `pom.xml`은 project root 안의 `<module>` POM을 제한된 깊이와 개수로 재귀
스캔한다. module 경로 이탈, cycle, 누락 파일, 제한 초과는 빈 성공 보고서 대신
실패 처리한다. 같은 reactor 안에서 group/artifact/version이 정확히 일치하는 module
의존성은 외부 package 목록에서 제외한다. 직접 의존성 중 버전이 명시되어 있거나 같은 파일 또는 일치하는
aggregator parent의
...
일반 filesystem `scan`, `ci`, 그리고 `diff`의 현재·기준선 양쪽은 로컬에 없는 exact parent/imported BOM을 고정된
Maven Central에서 각 side마다 최대 32개까지 자동 조회한다. 각 POM은 2 MiB, 전체 model depth는
...
압축파일 입력, project가 선언한 외부 또는 대체 Maven repository의 원격
...
attribute가 있는 flat package entry를 스캔하고, `.csproj` 파일에서는 직접
`PackageReference`와 `PackageDownload`를 스캔한다. `PackageDownload`의 exact range가
같은 파일의 unconditional property 하나만 참조하면 그 literal 값을 해석한다. `.csproj`가 NuGet Central Package Management를
...
저장소 전체 자동 탐색에서는 pin이 없는 environment 파일을 건너뛰지만 명시적
`--lockfile` 선택은 계속 엄격하다.
...
resolved version이 아니므로 package version인 척하지 않는다. 저장소 전체 자동 탐색에서는
해석할 수 없는 manifest를 건너뛰지만 명시적 `--lockfile` 선택은 계속 엄격하다. evidence는 로컬
...
evidence는 먼저 lockfile에 들어 있는 package license 선언을 사용하고, 선언이 없으면
로컬 `vendor/<vendor>/<package>/composer.json`과 license 파일에서 읽는다.
Composer plugin/platform repository 해석과 Packagist 원격 artifact fetch는
아직 지원하지 않는다.
...
패키지 metadata와 인식 가능한 license file은 서로 독립된 주장으로 대조한다.
...
PR에서 새로 들어오거나 바뀌거나 해결된 리스크를 보려면 baseline git ref와 비교한다.

```bash
ohrisk diff main --prod
```

diff는 baseline ref에서 락파일을 읽어 현재 결과와 비교한다. finding ID로 같은
대상을 찾고 fingerprint가 달라지면 changed로, 현재에만 있으면 new로, baseline에만
있으면 resolved로 분류한다. 심각도, 권장사항, 이유(reason), 증거(evidence)는
fingerprint에 포함되지만 action 안내 문구만 바뀐 경우에는 diff 노이즈를 내지 않는다.
...
| LGPL, EPL | review | review | 약한 카피레프트. 출시 전 검토 권장. |
...
corpora/dataset 같은 data에만 적용되는 제한은 finding evidence에
