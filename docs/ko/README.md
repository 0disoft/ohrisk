...
Maven 좌표는 로컬 POM evidence를 쓸 수 없을 때 고정된 공식
`repo.maven.apache.org` host에서 정확한 버전의 POM과 최대 8단계 parent POM을
조회한다. POM identity, XML, 응답 크기, timeout, redirect, DNS/socket 주소를 검증하고
npm registry token은 전달하지 않는다.
...
읽으며, 로컬 POM이 없으면 고정된 Maven Central exact-version 경로를 사용한다.
dependency version 해석을 위한 원격 parent/BOM fetching, 로컬 `.m2/repository` 밖의 외부 Maven repository resolution,
